...
Read all `team/*/activity/**/*.md` files for entries from the past 7 days, **excluding `team/*/activity/private/**`** — that folder is gitignored, local-only, and never goes into a shared retro. Extract cross-person patterns:
...
- **Activity logs for cross-team patterns** — session volume, work types, context-switching
